...
Each endpoint is a thin wrapper: fetch from cache → compute → sanitize → respond.
All heavy lifting is in analysis.py, all data fetching is in cache.py.
"""
...
from .cache import (
    ...
    get_history, get_quotes, clear_cache,
)
...
def _validate_symbol(symbol: str) -> str:
    s = symbol.strip().upper()[:10]  # cap length, uppercase
...
def api_info(symbol: str):
    ...
    try:
        ...
        return safe_json({k: info.get(k) for k in keys})
...
def api_buffett_ratios(symbol: str):
    ...
    try:
        ...
        if fin is None or fin.empty:
            raise HTTPException(status_code=503, detail="Financial data temporarily unavailable. Try again shortly.")
        info = get_ticker_info(sym)
        info["symbol"] = sym
...
def api_cluster(symbol: str):
    ...
    try:
        ...
        for peer in peers:
            try:
                p_fin, p_bs, p_cf = get_statements(peer)
                p_info = get_ticker_info(peer)
